...
namespace Rental.Documentation.WIP.BackUp
{
    public class UpdateCoverageBackUp
    {
        public RentalContext Db = new RentalContext();
        public int ProviderId { get; set; }
        public int SpecialistId { get; set; }

        public UpdateCoverageBackUp(int providerId, int specialistId)
        {
        ...
        public void GetCurrentProperties<T>(T objectInstance, Type objectType)
        {
        ...
        {
            System.Web.HttpContext.Current.Response.Write(String.Format("-------Getting the values of -{0}--------<br/>", coverageType));
            GetCurrentProperties(providerCoverage, coverageType);

            var difference =
                EnumeratePropertyWithTrueValue(specialistCoverage, coverageType);
            ...
            if (propertyInfos.Count() >= 0)
            {
                ...
                System.Web.HttpContext.Current.Response.Write("------Removing the values-------");
                System.Web.HttpContext.Current.Response.Write("<br/>");
                RemoveSpecialistCoverageFromProviderCoverage(propertyListTheNeedToBeRemovedFromProvider, coverageType);
                System.Web.HttpContext.Current.Response.Write("<br/>");
                System.Web.HttpContext.Current.Response.Write("----------PROVIDER AFTER CHANGE-------");
                System.Web.HttpContext.Current.Response.Write("<br/>");
                GetCurrentProperties(providerCoverage, coverageType);
            }
        }
        ...
        public void AddCoverageToTeam( object specialistCoverage, object providerCoverage, Type coverageType)
        {
            ...
            GetCurrentProperties(providerCoverage, coverageType);
            var difference =
                EnumeratePropertyDifferences(specialistCoverage, providerCoverage, coverageType);
            var propertyInfos = difference as PropertyInfo[] ?? difference.ToArray();
            if (propertyInfos.Count() >= 0)
            {
